...
function multiplayer.startServer()
    if not socket_ok then
        errorMessage = "LuaSocket not installed!\nPlease install luasocket."
        return false
    end
    local os = love.system.getOS()
    if os == "Windows" then
        -- Проверяем, есть ли server.exe (скомпилированный)
            ...
            if ok and parsed then
                ...
                elseif parsed.type == "update" then
                    ...
                    if playerId and players[playerId] then
                        ...
                        localPlayer.nick = p.nick or "Player"
                    end
                end
            end
        end
...
function multiplayer.draw()
    ...
    for id, p in pairs(players) do
        if id ~= playerId then
            local nick = p.nick or "Player" .. id
            love.graphics.setColor(0.8, 0.3, 0.3, 1)
            love.graphics.rectangle("fill", p.x - 25, p.y - 25, 50, 50)
            love.graphics.setColor(1,1,1,1)
            love.graphics.setFont(love.graphics.newFont(14))
            love.graphics.printf(nick, p.x - 50, p.y - 45, 100, "center")
        end
    end

    -- Локальный игрок
    local nick = localPlayer.nick or "YOU"
    love.graphics.setColor(0.3, 0.8, 0.3, 1)
    love.graphics.rectangle("fill", localPlayer.x - 25, localPlayer.y - 25, 50, 50)
    love.graphics.setColor(1,1,1,1)
    love.graphics.setFont(love.graphics.newFont(14))
    love.graphics.printf(nick, localPlayer.x - 50, localPlayer.y - 45, 100, "center")
